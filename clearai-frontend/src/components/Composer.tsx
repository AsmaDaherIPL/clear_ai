      ...
      {mode === 'batch' && (
        <div className="p-4">
          ...
          <div
            ...
            className={cn(
              'rounded-[var(--radius)] border-[1.5px] border-dashed',
              'py-14 px-6',
              'flex flex-col items-center gap-3 text-center',
              'transition-[border-color,background] duration-150',
              isDragOver
                ? 'border-[var(--accent)] bg-[oklch(0.97_0.04_60)]'
                : 'border-[#ddd3c9] bg-[oklch(0.97_0.025_55)]',
            )}
          >
            {/* File icon in white pill */}
            <div className="w-[52px] h-[52px] rounded-[14px] bg-[var(--surface)] shadow-[0_2px_8px_rgba(35,25,21,0.10)] flex items-center justify-center">
              <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="var(--accent)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <div>
              <h3 className="m-0 text-[18px] font-semibold tracking-[-0.01em] text-[var(--ink)]">
                {batchFile ? batchFile.name : t('drop_manifest_title')}
              </h3>
              <p className="m-0 mt-1 text-[13px] text-[var(--ink-3)]">
                {batchFile
                  ? `${(batchFile.size / 1024).toFixed(1)} KB · ready to submit`
                  : t('drop_manifest_hint')}
              </p>
              {batchError && (
                <p className="m-0 mt-1 text-[13px] text-[var(--accent-ink)]" role="alert">
                  {batchError}
                </p>
              )}
            </div>
            ...
            <div className="mt-1 flex items-center gap-2.5">
              <button
                ...
                className={cn(
                  ...
                  'shadow-[0_4px_12px_-3px_rgba(233,123,58,0.45)]',
              ...
              {batchFile && (
                <button
                  ...
                  className={cn(
                    'border border-[var(--line)] bg-[var(--surface)] text-[var(--ink-2)]',
                    'px-5 py-2.5 rounded-full text-[13.5px] font-medium',
                    'hover:border-[var(--ink-3)] hover:text-[var(--ink)] transition-colors duration-150',
                    'disabled:opacity-50 disabled:pointer-events-none',
                  )}
                >
          ...
          {/* CSV template link — below the dropzone card */}
          <div className="mt-3 flex justify-end">
            <a
              href="/templates/clearai-batch-template.csv"
              download
              className="text-[13px] text-[var(--accent-ink)] hover:underline font-medium"
            >
              {t('drop_csv_template')} →
            </a>
          </div>
