...
export const DEPLOYMENT = {
  ...
  resources: [
    ...
    { name: 'Storage Account `stinfpclearaidevgwc01` (Standard_LRS, MI-only)',     note: 'Container `declaration-runs/` stores per-run input.csv + classifications.json + run-index.json + HV/LV XMLs under {operatorSlug}/YYYY/MM/DD/{runId}/. allowSharedKeyAccess=false; allowBlobPublicAccess=false; SPA downloads via 5-min user-delegation SAS URLs minted by the backend MI.' },
