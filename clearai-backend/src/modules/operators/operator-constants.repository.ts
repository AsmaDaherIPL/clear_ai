...
export async function getConstantsAsRecord(operatorId: string): Promise<Record<string, string>> {
  const rows = await db().select().from(operatorConstants).where(eq(operatorConstants.operatorId, operatorId));
