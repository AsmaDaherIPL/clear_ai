/**
 * Pipeline rewrite — discriminated-union contracts (PR 1).
 *
 ...
 *
 * Target architecture (per pipeline-rewrite-plan-2026-05-15.md):
 *
 *   parse → (identify_fast || merchant_resolution) → maybe identify_web
 *     → scope_selection → multi_arm_retrieval + union + rerank
 *     → picker → verifier → (submission || sanity) → DispatchResult
 *
 ...
 * probes.
 *
 * Lives under v2/ during the rewrite (PRs 1-12). PR 13 promotes this
 * directory to be the canonical pipeline/ and deletes the legacy +
 * anchored counterparts. See the rewrite plan.
 */
...
// Re-exports for convenience — keeps imports inside v2/ to a single
// module without forcing callers to know which symbols came from legacy.
export type {
  CanonicalLineItem,
  MerchantCodeState,
  ParsedItem,
...
export interface PipelineTraceV2 {
