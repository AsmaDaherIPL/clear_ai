/**
 * Thin HTTP layer for batch endpoints. Multipart parse + zod
 * validation + delegation to batch.use-case. Maps errors to the
 * shared envelope.
 */
...
export async function handleGetBatch(req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply): Promise<unknown> {
  ...
  const operator = await getOperatorById(batch.operatorId);
  const summary: BatchSummary = {
    id: batch.id,
    operator_slug: operator?.slug ?? '',
    mode: batch.mode as BatchMode,
    status: batch.status as BatchStatus,
    classification_status: batch.classificationStatus as ClassificationStatus,
    declaration_status: (batch.declarationStatus ?? null) as DeclarationStatus | null,
...
export async function handleListClassifications(
  ...
  const listParams: unknown[] = [batch.id, limit, offset];
  ...
  // on the indexed batch_id); short-circuiting would require
  // caching and complicate the freshness story during live ingestion.
  const countParams: unknown[] = [batch.id];
  ...
  return reply.send({
    ...
    // 2026-05-12 API cutover. The DB column is now
    // batch_items.batch_id; the wire-format name matches.
    batch_id: batch.id,
    operator_slug: operatorSlug,
    classification_phase: batch.classificationStatus,
    total,
    limit,
    offset,
    has_more: hasMore,
