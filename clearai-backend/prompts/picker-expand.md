...
```json
{
  "verdicts": [
    {
      "code": "010121100000",
      "fit": "fits" | "partial_family" | "does_not_fit",
...
If the leaf constrains a dimension (e.g. "of cotton") and the description is silent on that exact dimension, that's `partial_family` — the leaf might be right, but the description doesn't prove it.
...
- **partial_family**: chapter+heading right; leaf constrains a dimension the description is silent on or only partially matches.
