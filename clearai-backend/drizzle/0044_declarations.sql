...
--
-- One row per rendered ZATCA Declaration bundle. Phase 2 of the pipeline
-- (modules/declaration-sets/declaration/) inserts a row for every bundle it
-- produces — HV (one item) or LV (up to tenant.bundleSize items).
--
-- Why a dedicated table rather than a column on declaration_sets:
--   • A declaration_set produces N declarations (1 per HV item +
--     ceil(LV/bundleSize)). The relation is genuinely one-to-many, not
--     metadata.
...
--   • blob_key is per bundle; the result_blob_key on declaration_sets is a
--     v0 convenience pointer, not the source of truth.
--
-- What's safe:
...
CREATE TABLE IF NOT EXISTS declarations (
  ...
  -- Owning declaration_set. ON DELETE CASCADE — deleting a set deletes its
  -- declaration rows.
  ...
  -- (e.g. declaration-sets/{setId}/declarations/0001.xml).
  ...
  CONSTRAINT declarations_set_fk
  ...
  CONSTRAINT declarations_set_bundle_uniq
    UNIQUE (declaration_set_id, bundle_index)
);
--> statement-breakpoint

CREATE INDEX IF NOT EXISTS declarations_set_idx
  ON declarations (declaration_set_id);
--> statement-breakpoint
