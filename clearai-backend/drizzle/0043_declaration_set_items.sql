-- ============================================================================
-- 0043_declaration_set_items.sql
--
-- One row per parsed line item under a declaration_set. Phase 1
-- (classification) reads pending rows, calls dispatch(item), and writes the
-- result back into classification_result + final_code + trace + status.
--
...
--     IDs, phone numbers from the source upload). Embedding it inside
--     `canonical` would force every operational read of canonical to drag
--     the PII out wholesale.
...
--     `SELECT canonical, raw_row FROM declaration_set_items WHERE id = $1`.
--
...
CREATE TABLE IF NOT EXISTS declaration_set_items (
  id                    uuid          PRIMARY KEY DEFAULT gen_random_uuid(),

  -- Parent declaration_set. ON DELETE CASCADE — deleting a set deletes its
  -- items (matches the rule-4 contract for batch-style child tables).
  declaration_set_id    uuid          NOT NULL,

  -- 1-based row position from the source file (post-header). UNIQUE per
  -- (declaration_set_id, row_index) so retries don't accidentally duplicate
  -- rows.
  ...
  CONSTRAINT declaration_set_items_set_fk
    FOREIGN KEY (declaration_set_id) REFERENCES declaration_sets(id) ON DELETE CASCADE,

  CONSTRAINT declaration_set_items_final_code_fk
    FOREIGN KEY (final_code) REFERENCES zatca_hs_codes(code) ON DELETE RESTRICT,

  CONSTRAINT declaration_set_items_status_chk
    CHECK (status IN ('pending', 'classifying', 'succeeded', 'flagged', 'blocked', 'failed')),

  CONSTRAINT declaration_set_items_row_index_nonneg_chk
    CHECK (row_index >= 1),

  CONSTRAINT declaration_set_items_canonical_object_chk
    CHECK (jsonb_typeof(canonical) = 'object'),

  CONSTRAINT declaration_set_items_raw_row_object_chk
    CHECK (jsonb_typeof(raw_row) = 'object'),

  CONSTRAINT declaration_set_items_classification_result_object_chk
    CHECK (classification_result IS NULL OR jsonb_typeof(classification_result) = 'object'),

  CONSTRAINT declaration_set_items_trace_object_chk
  ...
  CONSTRAINT declaration_set_items_final_code_format_chk
  ...
  CONSTRAINT declaration_set_items_final_code_status_consistency_chk
    CHECK (
  ...
  CONSTRAINT declaration_set_items_set_row_uniq
    UNIQUE (declaration_set_id, row_index)
);
--> statement-breakpoint

-- Composite (declaration_set_id, row_index) covers two hot read paths:
--   1. WHERE declaration_set_id = $1                    (leftmost-prefix btree lookup)
--   2. WHERE declaration_set_id = $1 ORDER BY row_index (index scan satisfies
--      ORDER BY, no in-memory sort)
-- A single-column (declaration_set_id) index would force Postgres to sort
-- after the scan; for sets with thousands of items that's measurable. The
-- composite is free at table-creation time so we pay the cost once.
CREATE INDEX IF NOT EXISTS declaration_set_items_set_row_idx
  ON declaration_set_items (declaration_set_id, row_index);
--> statement-breakpoint

-- Hot-path partial index for the Phase 1 worker's claimNextItem query
-- (SELECT ... WHERE declaration_set_id = $1 AND status = 'pending' ...).
CREATE INDEX IF NOT EXISTS declaration_set_items_pending_idx
  ON declaration_set_items (declaration_set_id)
...
CREATE INDEX IF NOT EXISTS declaration_set_items_final_code_idx
  ON declaration_set_items (final_code)
  WHERE final_code IS NOT NULL;
--> statement-breakpoint

DROP TRIGGER IF EXISTS declaration_set_items_touch_updated_at_trg ON declaration_set_items;
--> statement-breakpoint

-- Reuses batches_touch_updated_at() defined in 0038.
CREATE TRIGGER declaration_set_items_touch_updated_at_trg
  BEFORE UPDATE ON declaration_set_items
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'clearai_app') THEN
    GRANT SELECT, INSERT, UPDATE ON declaration_set_items TO clearai_app;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'clearai_readonly') THEN
    ...
    GRANT SELECT (
      id, declaration_set_id, row_index,
      canonical,
      status, final_code,
      classification_result, trace, error,
      created_at, updated_at
      -- intentionally NOT granted: raw_row (PII).
    ) ON declaration_set_items TO clearai_readonly;
  END IF;
END
$$;
--> statement-breakpoint
