...
describe('runDeclarationPhase', () => {
  it('produces HV bundles for items >= threshold and LV chunks for the rest', async () => {
    ...
    const { runDeclarationPhase } = await import('../../../src/modules/batches/filings/declaration.runner.js');
    ...
    const rows = await db()
      .select()
      .from(batchFilings)
      .where(eq(batchFilings.batchId, batchId))
      .orderBy(batchFilings.bundleIndex);
  ...
  it('excludes blocked and failed items', async () => {
    ...
    const rows = await db().select().from(batchFilings).where(eq(batchFilings.batchId, batchId));
  ...
  it('writes XML to the configured blob backend', async () => {
    ...
    const rows = await db().select().from(batchFilings).where(eq(batchFilings.batchId, batchId));
    expect(rows[0]!.blobKey).toMatch(new RegExp(`declaration-runs/${batchId}/declarations/0000\\.xml`));
