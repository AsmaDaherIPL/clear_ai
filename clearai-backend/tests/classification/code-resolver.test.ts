...
describe.skipIf(!process.env.RUN_DB_TESTS)('runCodeResolver — override feeds codebook walk', () => {
  afterAll(async () => {
    await closeDb();
  });
  ...
  it('no override + active 12-digit code → passthrough', async () => {
    ...
    const r = await runCodeResolver('851629100000', 'twelve_digit', 'space heater', 'naqel');
  ...
  it('no override + 10-digit prefix unknown → walks down to 6 and resolves', async () => {
    ...
    const r = await runCodeResolver('8516299100', 'short_prefix', 'space heater', 'naqel');
  ...
  it('override hit + target is active 12-digit → passthrough, override_applied=true', async () => {
    ...
    const r = await runCodeResolver('61082100', 'short_prefix', 'cotton garment', 'naqel');
