...
| PR16 | Entropy formula fix: per-candidate fit weights. PR9's distribution computation used a pool-wide `avgFitWeight` constant on every candidate's rerank contribution — which **cancels in normalisation**, leaving the distribution dependent on rerank scores alone. Today's 9-row pilot exposed the bug: every row landed at 0.05 / `low` because rerank scores cluster tightly (~0.117-0.169 range) and the picker's per-candidate `fit` verdict was discarded. Fix: thread the `verdicts` array into `computeConfidence` so each candidate's rerank is multiplied by its **own** fit weight (fits=1.0, partial=0.5, does_not_fit=0.10). Smoke-tested against the #1 thermos pool: confidence rises from 0.05 → 0.26 (fair) — honest reflection of "clean fit with 2 partial competitors." Backward-compat: `verdicts` is optional; PR9 fallback path preserved for tests that pre-date PR16. 3 new tests pin the new behaviour. pipeline-v2 223/223 green. | pending (rev TBD) |
