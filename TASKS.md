...
| PR11 | Bare-noun audit gate (TASKS S2 #13). New `detectBareNounRisk` helper: tokenizes raw input description after stripping punctuation, numbers, units (ml/kg/pcs/SAR), stopwords, SKU-shaped tokens. When <3 significant tokens remain AND pick.fit !== 'fits' → fires `audit_flag = true` on the winner. Catches "Trimmer", "Bracelet", "playmat", "هودي فضفاض" style misclassifications that look-clean-but-aren't. Reshaped from the original `identity_tokens.length < 2` proposal after pilot evidence showed identify expands bare nouns into 2-3 semantic tokens (so the raw input shape is the right signal). Audit-only — XML still ships, row routes to HITL. 9 new tests; pipeline-v2 212/212 green. | `1e829f2` (rev 0000165) |
| PR12 | Humane sanity rationale. Splits sanity output into `rationale_short` (one human-readable sentence, SPA-facing — e.g. "800 SAR is much higher than typical for an unbranded mug (20-100 SAR). About 8 times the upper end.") + `rationale_detail` (the math, engineer-facing — e.g. "band 20-100 SAR; 800/100=8.0×; outside [0.2,5.0]"). Applied to both PASS and FLAG for consistency. Value-only (no diagnostic causes per audit-only rule). Legacy `rationale` field preserved on the wire as a backward-compat mirror of `rationale_short` → SPA gets the human-readable form without any frontend migration. PR8 reconciliation logic now reads `rationale_detail` (math) for the verdict check. pipeline-v2 212/212 green. | `a93865b` (rev 0000165) |
| PR13 | Winner full breadcrumb in `resolved_hs_code_description.full_hierarchy`. Previously when `zatca_hs_code_display.path_en` was a single segment (heading-level codes with no displayable ancestor chain), the API returned just the leaf label (e.g. "Men's or boys' shirts"). PR13 reconstructs the full breadcrumb from `path_codes` ancestor labels when `path_en` is short. Applied in two places: `lookupCatalogPath()` (single-shot dispatch path) and the batch controller's SQL (LATERAL join with `string_agg`). Annotated candidates already had full paths (no change there). pipeline-v2 212/212 green. | `75ec1f6` (rev 0000166) |
| PR14 | Band floor on accepted codes. `deriveConfidenceBand` gains an `acceptedContext: boolean` parameter — when `true`, the floor clamps to `low` instead of `no_result`. Reserves `no_result` for genuine escalates (no code shipped). Today's real case: "cotton shirt men" → picker accepted code 620500000000 with entropy confidence 0.05, but SPA showed `no_result` even though the code shipped, verifier flagged UNCERTAIN, and sanity PASSed. PR14 makes that row show `low` instead. Per-candidate annotated-candidate callers keep the default behaviour (a loser can legitimately be `no_result`). pipeline-v2 214/214 green. | `7b4f499` (rev 0000167) |
| PR15 | Band-based HITL routing. New policy from user (2026-05-20): only `high` and `moderate` bands bypass review; `fair`, `low`, and `no_result` always route to HITL. Adds `low_confidence_band` to `HitlPayload.reason` union and DB CHECK (migration 0088). buildHitl() fires it as a last-resort when no more-specific reason (sanity_flag / verifier_uncertain / verdict_escalate / brand-rescue) matched. classification_status is also flipped to `DRIFT` (not `AGREEMENT`) when an accepted pick's band is below moderate, keeping status + HITL routing consistent. 6 new orchestrator tests cover band-by-band routing + sanity_flag precedence. pipeline-v2 220/220 green. | `4f6b298` (rev 0000168) |
| PR16 | Entropy formula fix: per-candidate fit weights. PR9's distribution computation used a pool-wide `avgFitWeight` constant on every candidate's rerank contribution — which **cancels in normalisation**, leaving the distribution dependent on rerank scores alone. Today's 9-row pilot exposed the bug: every row landed at 0.05 / `low` because rerank scores cluster tightly (~0.117-0.169 range) and the picker's per-candidate `fit` verdict was discarded. Fix: thread the `verdicts` array into `computeConfidence` so each candidate's rerank is multiplied by its **own** fit weight (fits=1.0, partial=0.5, does_not_fit=0.10). Smoke-tested against the #1 thermos pool: confidence rises from 0.05 → 0.26 (fair) — honest reflection of "clean fit with 2 partial competitors." Backward-compat: `verdicts` is optional; PR9 fallback path preserved for tests that pre-date PR16. 3 new tests pin the new behaviour. pipeline-v2 223/223 green. | `86998d6` (rev 0000169) |
| PR17 | Sanity thin-description PASS rule + high-value carve-out. Adds a pre-check before band selection: when raw_description has no brand, no SKU/model, and no material qualifier (bare nouns like "Boots", "Bag", "Bracelet", "Shirt"), emit PASS with rationale "Generic <product> — value plausibility cannot be assessed." Carve-out: high-value categories (gold/silver/jewelry chapter 71 + branded watches chapter 91) still go through the normal band check because cheap-fake undervaluation in those classes is the real customs-risk pattern. Today's "Boots" 18.75 SAR row triggered the change — sanity FLAGged it as "boots typically 150-800 SAR", but bare-noun footwear at 18 SAR can legitimately be a cheap import. Prompt-only change; sanity is a pure LLM call so no TS code needed. pipeline-v2 223/223 green. | `7ee1606` (rev 0000170) |
